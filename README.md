...
AVL trees are a type of self-balancing binary search tree where the heights of the left and right subtrees of any node differ by at most one. The difference in the heights of the left and right subtrees is known as the balance factor b(v). The formula below shows the balance factor of a node v in an AVL tree [1].
...
An example or rotation is display below.
